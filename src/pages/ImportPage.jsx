...
const SAMPLE = 'Style Number,Brand,Pattern Number,Style Type,Style Primary Color,Fabric Type,Style Name,MRP,Style Status';
...
export default function ImportPage({ onSuccess }) {
  ...
  const pick = (f) => { setFile(f); setResult(null); setError(''); };
  const onFile = (e) => { if (e.target.files?.[0]) pick(e.target.files[0]); };
  const onDrop = (e) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await apiFetch('/import', { method: 'POST', body: JSON.stringify({ csv, delimiter }) });
      setResult(res.data);
      setFile(null);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Upload styles in bulk. Re-importing the same Style Number updates the record.</p>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">Required Format</span>
        </div>
        <div className="space-y-2 text-sm text-slate-600">
          <p>• First row must be <strong>headers</strong></p>
          <p>• Column named <strong>"Style Number"</strong> is required (used as unique ID)</p>
        ...
        <div className="mt-4 bg-white border border-slate-200 rounded-xl px-4 py-3">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Example header row</p>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 space-y-5">
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition ${
            dragging ? 'border-slate-800 bg-slate-50' : 'border-gray-200 hover:border-slate-400 hover:bg-gray-50'
          }`}
        >
          ...
          {file ? (
            ...
            <>
              ...
              <p className="text-sm font-medium text-gray-500">Drop your CSV here or <span className="text-slate-800 font-semibold">click to browse</span></p>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2">
            {[',', ';', '\t'].map((d) => (
              <button key={d} type="button" onClick={() => setDelimiter(d)}
                className={`px-3 py-1.5 rounded-lg text-xs font-mono font-semibold border transition ${
                  delimiter === d ? 'bg-slate-900 text-white border-slate-900' : 'border-gray-200 text-gray-600 hover:border-slate-400'
                }`}
              >
                {d === '\t' ? 'TAB' : `"${d}"`}
              </button>

/**
 * Myntra blazers Listing Generator
 * Generates a Myntra-compatible listing CSV for Blazer category products.
 */
...
// Default CSV headers matching Myntra's blazer template
export const MYNTRA_BLAZER_HEADERS = [
  'styleId',
  'styleGroupId',
  'vendorSkuCode',
  'vendorArticleNumber',
 ...
 */
export function generateMyntraBlazerListing(
  selectedData,
  sizeMapping = MYNTRA_SIZE_MAPPING,
  csvHeaders = MYNTRA_BLAZER_HEADERS,
  customMaps = {},
  userDetails = {}
) {
  ...
  const blazerCategoryFilter = (product) => {
    const f = product.fields || {};
    const type = (f.style_type || f.styleType || f.category || f.type || '').toLowerCase();
    return type.includes('blazer') || type.includes('blazers');
  };

  const csvData = selectedData.filter(blazerCategoryFilter).flatMap((product, index) => {
    const f = product.fields || {};
    const get = (...keys) => {
  ...
  if (csvData.length === 0) {
    throw new Error('No blazer-category products found in selected data.');
  }

  downloadCsv('Myntra_Blazers_listing.csv', csvHeaders, csvData);
